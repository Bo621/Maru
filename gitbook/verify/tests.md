...
## E2E: 15개 (실제 GIWA Sepolia 상대)

기본 게이트 14개 + 실서비스 스모크 1개(`@smoke` 태그, 업비트를 실제로 불러
판정을 확인).
...
| S6 | 스레드 배지 → 상세 → 이전 판단 스레드 섹션(노드 2개 이상) |
| S7 | 검증을 통과한 이유 원문에만 해시 일치 표시가 붙는다 |
| S8 / S8-보강 | 관측 종료 결정에 맞음·틀림, 업비트를 못 불러도 스냅샷으로 판정 |
| S9 | 팔로우가 실제로 거른다 |
| S10 / S10-보강 | 곧 결과 나옴 정렬, 곧 결과 나옴↔전체 전환과 URL 유지 |
...
Playwright는 로컬 anvil이 아니라 **실제 GIWA Sepolia**를 상대로 돈다. 공개 RPC
응답을 기다렸다가 피드가 최종적으로 렌더되는지를 확인한다. 로딩 상태 자체가
먼저 보이는지는 검사하지 않는다.
