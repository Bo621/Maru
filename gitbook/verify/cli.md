...
| `1` | `MISMATCH` | 일치하지 않음 |
| `2` | (없음) | 사용법 오류 또는 실행 자체가 실패 |
| `3` | `NO_SETTLEMENT` / `NO_OBSERVATION` / `NOT_REQUIRED` | 검증할 대상이 없다. **틀렸다는 뜻이 아니다** |

`3`으로 묶이는 세 판정은 각각 "발행자가 아직 결과를 안 올림"·"관측값을
아직 못 구함"·"예상 결과를 선언하지 않아 검증 자체가 필요 없음"이다.
셋 다 `1`(MISMATCH)로 묶으면 "아직 안 올림"과 "틀림"이 뭉개진다.
**검증하지 못한 것과 틀린 것은 다르다.**
