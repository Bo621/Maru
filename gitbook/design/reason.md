...
이 값이 거짓이면 시점 배지를 렌더하지 않는다. 해시 일치와 시점은 서로 다른
근거에서 나오므로, 하나의 배지에 두 주장을 섞지 않는다.

**재사용(replay)은 막히지 않는다.** 커밋 프리이미지는 결정 UID·부모·구간·지표를
...
| 컴포넌트 언마운트 | **공유 요청을 취소하지 않는다** — 언마운트한 호출자만 결과를 버린다 |

마지막 행이 중요하다. 요청 중복 제거와 `AbortController` 취소를 같이 쓰면
한 호출자의 언마운트가 다른 모든 호출자의 요청을 죽인다. `signal`을 API에서
아예 받지 않게 만들어 이 문제를 구조적으로 막는다.
